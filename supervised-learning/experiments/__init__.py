...
__all__ = ['ANN', 'Boosting', 'DT', 'KNN', 'SVM', 'DTPost']
...
def basic_results(clf, classes, training_x, training_y, test_x, test_y, params, clf_type=None, dataset=None,
    ...
    plt = plot_learning_curve('Learning Curve: {} - {}'.format(clf_type, dataset_readable_name),
                              train_sizes / n,
                              train_scores, test_scores, x_label="Training Data Size (% of total)")
    plt.savefig('{}/images/{}_{}_LCP.png'.format(OUTPUT_DIRECTORY, clf_type, dataset), format='png', dpi=150)

    logger.info(" - Learning curve complete")

    return cv

def post_prune(clf, classes, training_x, training_y, test_x, test_y, params, clf_type=None, dataset=None,
                  dataset_readable_name=None, balanced_dataset=False, best_params=None, seed=55, threads=1):
    logger.info("Computing post-prune results for {} ({} thread(s))".format(clf_type, threads))
    ...
    if best_params:
        ...
        cv.fit(training_x, training_y)
        reg_table = pd.DataFrame(cv.cv_results_)
        reg_table.to_csv('{}/{}_{}_reg_post.csv'.format(OUTPUT_DIRECTORY, clf_type, dataset), index=False)
        test_score = cv.score(test_x, test_y)
        ...
        final_estimator.write_visualization('{}/images/{}_{}_LC_full'.format(OUTPUT_DIRECTORY, clf_type, dataset))
...
def make_post_prune_complexity_curve(x, y, param_name, param_display_name, param_values, clf, clf_name, dataset,
                          dataset_readable_name, x_scale, verbose=False, balanced_dataset=False, threads=1):
    logger.info("Building model complexity curve")
    curr_scorer = scorer
    if not balanced_dataset:
...
def perform_experiment(ds, ds_name, ds_readable_name, clf, clf_name, clf_label, params, timing_params=None,
    ...
    pipe = ds.pipe(clf_label, clf)

    ds_final_params = None
    if not iteration_lc_only:
        print("here")
        if clf_name == 'DTPost':
            ds_clf = post_prune(pipe, np.unique(ds.classes), ds_training_x, ds_training_y, ds_testing_x, ds_testing_y,
