...
def plot_combined(title, df, data_columns, tsne_data=None, extra_data=None, extra_data_name=None,
    ...
    for column in data_columns:
        if column == 'tsne':
            ...
            ln = ax1.plot(df.index.values, df[column], linewidth=1,
                     label=algos[column]['descriptive_name'])
            lns += ln
    if tsne_data is not None:
        ln = ax2.scatter(tsne_data['x'], tsne_data['y'], c=tsne_data['target'], alpha=0.7, s=5)
    ...
    if extra_data is not None and extra_data_name is not None:
        ex_ax = ax1.twinx()
        ln = ex_ax.plot(extra_data.index.values, extra_data.iloc[:, 0], '-r', linewidth=1,
